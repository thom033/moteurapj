...
public class UserEJBBean implements UserEJB {
    ...
    public MapUtilisateurServiceDirection testeValide(String user, String pass, Connection connection) throws Exception {
        try {
            historique.UtilisateurUtil uI = new UtilisateurUtil();
            return (MapUtilisateurServiceDirection) uI.testeValide(user, pass,connection);
        } catch (Exception ex) {
            ex.printStackTrace();
            throw new Exception(ex.getMessage());
        }
...
//    }
    ...
    @Override
    public void testLoginGallois(String user, String pass, String interim, String service) throws Exception {
        Connection c = null;
        try {
    ...
    @Override
    public Object createObjectMultipleGallois(ClassMAPTable mere, String colonneMere, ClassMAPTable[] fille) throws Exception {
        Connection c = null;
        Object ret = null;
        try {
            c = new UtilDB().GetConn("gallois","gallois");
    ...
    @Override
    public Object createObjectMultipleGallois(ClassMAPTable mere, String colonneMere, ClassMAPTable[] fille,Connection connection) throws Exception {
        Object ret = null;
        ...
        try {
            ...
            Object idmere = createObject(mere, connection);
            String id = null;
            if(idmere!=null){
                id = ((ClassMAPTable) idmere).getTuppleID();
            }
            ret = createObjectMultiple(fille, colonneMere, id, connection);
            ret = idmere;
            return ret;
        } catch (Exception ex) {
            ex.printStackTrace();
...
//                return o;
    ...
    @Override
    public void logTo(String username, String password, Connection connection) throws Exception {
        Connection c = connection;
        try {
            c.setAutoCommit(false);
            u = testeValide(username, password, c);
            UtilisateurUtil crt = new UtilisateurUtil();
            uVue = crt.testeValide("utilisateurVue", username, password,connection);
            type = u.getIdrole();
            if (type.compareToIgnoreCase(bean.Constante.getIdRoleDirecteur()) == 0) {
                Direction d[] = findDirection("", "", "", "", String.valueOf(u.getRefuser()));
                if (d.length > 0) {
